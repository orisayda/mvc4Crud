...
app.controller('TableEventListController', ['$scope','$http','$state','$stateParams',function($scope, $http, $state, $stateParams,$filter) {
    ...
    function getDatecheck(){
      ...
      if(abcdate.getMonth()+1 < 10){
        
        month = '0'+(abcdate.getMonth()+1);
      }else{
          month = (abcdate.getMonth()+1);
      }

      abcdate = abcdate.getFullYear() +'-'+ month +'-'+ (abcdate.getDate());
    ...
    $scope.$on('$stateChangeSuccess', function (ev, to, toParams, from, fromParams) {
      // console.log (from.name,11);
      $scope.checkRoute = from.name;
    ...
    function getData(){
      if($scope.checkRoute != "app.event_list" || $scope.checkRoute != "access.signin" || $scope.checkRoute != "access.signup" || $scope.checkRoute != ""){
        $scope.tasks = eventList;        
      }
    }

    changeSerachValue();

    function changeSerachValue(){
      var item = document.getElementById("search_id");
      // console.log(item);
       $scope.searchText = item.dataset.searchvalue;

       var statusItem = document.getElementById("status_id");

       if(statusItem.dataset.statusvalue != "1"){

          if(statusItem.dataset.statusvalue == "un-completed"){
            $scope.showCompleted = "Show All Tasks";
          }else{
            $scope.showCompleted = "Hide Completed Tasks";
          }
    ...
    function changeSortValue(){
       var item = document.getElementById("sortById"); 

       // console.log($scope.checkRoute,1);
       
       if(item.dataset.sortvalue == "title"){

            $scope.sortBy.selected = "title";
            if($scope.checkRoute == "app.event_list" || $scope.checkRoute == "access.signin" || $scope.checkRoute == "access.signup" || $scope.checkRoute == ""){
       ...
       }else if(item.dataset.sortvalue == "high_priority"){

            $scope.sortBy.selected = "high_priority";
            if($scope.checkRoute == "app.event_list" || $scope.checkRoute == "access.signin" || $scope.checkRoute == "access.signup" || $scope.checkRoute == ""){
              $scope.tasks = getTasks();
       ...
       }else if(item.dataset.sortvalue == "start_date"){

            $scope.sortBy.selected = "start_date";
            if($scope.checkRoute == "app.event_list" || $scope.checkRoute == "access.signin" || $scope.checkRoute == "access.signup" || $scope.checkRoute == ""){
              $scope.tasks = getTasks();
        ...
        // console.log(toDoEventRoutes);
            ...
            $scope.sortBy.selected = "event_id";
            if($scope.checkRoute == "app.event_list" || $scope.checkRoute == "access.signin" || $scope.checkRoute == "access.signup" || $scope.checkRoute == ""){
              $scope.tasks = getTasks();
       ...
       // console.log($scope.sortBy.selected);
       // console.log($scope.tasks,2);
    ...
    $scope.sortTasks = function(){     
      ...
      $stateParams.sortBy = $scope.sortBy.selected;
      // console.log($scope);
      // alert($rootScope.loggeIn);
      
      $state.go('app.event_list', {sortBy:$stateParams.sortBy}, {reload: true});

    }

    $scope.searchbystatus = function(){

      if($scope.showCompleted == "Hide Completed Tasks"){

        $stateParams.taskStatus = "un-completed";
        $scope.showCompleted = "Show All Tasks";             
        $state.go('app.event_list', {status:$stateParams.taskStatus}, {reload: true});
        

      }else{

        $stateParams.taskStatus = "all";
        $scope.showCompleted = "Hide Completed Tasks";           
        $state.go('app.event_list', {status:$stateParams.taskStatus}, {reload: true});               
    ...
    $scope.sort_priorityfunction = function(value){

      // console.log(value,$scope.sort_priority);

      if($scope.sort_priority != "select1"){

        // console.log($scope.sort_priority.selected);

        $stateParams.sortBy = $scope.sort_priority;      
        $state.go('app.event_list', {sortBy:$stateParams.sortBy}, {reload: true});
    ...
    $scope.search = function(){
        ...
        if($scope.searchText == ""){
          ...
          alert("Kindly Put Something to Search");
        
        }else{
          
        //searchEvent = $scope.searchText;
        
        $stateParams.searchEvent = $scope.searchText;
      ...
      $scope.reloadEventList = function(actionto,pageNumber){
       
       // console.log(actionto,pageNumber);

       // eventList = upList;

       // console.log(eventList);

       $stateParams.pageNumber = pageNumber ;
    ...
    $scope.editBook = function (event_id) {
      ...
      // console.log('controller',event_id);

      //var id = $(e.target).data('event_id');         

      $stateParams.event_id = event_id;
      
      // console.log($stateParams.event_id);      

      $state.go('app.event_edit');

      // $state.go('app.event_edit', {paramEventId:edit_event_id});


        // var getBookData = getEventById(event_id);
        ...
        //     $scope.start_time = $scope.eventData.returnStartTime;
        //     $scope.end_time = $scope.eventData.returnEndTime;
        ...
        //     ///$scope.countryAsync = {selected : "Albania"};

        //     // $scope.set_date($scope.start_date,$scope.end_date);   
    ...
    $scope.setSearchText = function(textserch){

      alert(textserch);
      console.log(textserch);
    ...
    // }
      ...
      $scope.deleteBook = function (event_id,item) {
        //'console.log(event.event_id);



       var getBookData = DelEvent(event_id);
        ...
        getBookData.then(function (msg) {
            ...
            var index = eventList.indexOf(item);

            eventList.splice(index, 1);  


            //$scope.tasks.push(updated_data);   
...
/*
    ...
    // $scope.set_date = function(start,end){
